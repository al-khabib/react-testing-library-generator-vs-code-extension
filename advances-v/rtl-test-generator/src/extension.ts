...
export function activate(ctx: vscode.ExtensionContext) {
  ...
  ctx.subscriptions.push(
    vscode.commands.registerCommand('rtl-test-generator.generateTest', () =>
      openConfigWebview(ctx)
    )
  )
  ...
  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      'rtlTestGeneratorView', // Must match the view ID in package.json
...
function openConfigWebview(ctx: vscode.ExtensionContext) {
...
async function generateAndInsertTest(payload: any) {
  ...
  if (editor) {
    ...
    vscode.window.showErrorMessage(
      'No active editor or workspace folder found.'
    )
    return
  }
  ...
  try {
    ...
    if (fileExists) {
      const overwrite = await vscode.window.showWarningMessage(
        `File ${testFileName} already exists. Overwrite?`,
        { modal: true },
        'Yes',
        'No'
      )
...
class RtlTestSidebarProvider implements vscode.WebviewViewProvider {
